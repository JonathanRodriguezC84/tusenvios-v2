    ...
    <div class="p-3 lg:p-5 h-full flex flex-col">
        ...
        <nav class="mt-3 shrink-0 overflow-x-auto" aria-label="Accesos rapidos por estado">
            <div class="flex min-w-max gap-2">
                <?php $__currentLoopData = $shipmentSummary['shortcuts']; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $shortcut): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                    <?php
                    ...
                    ?>
                    <a href="<?php echo e($shortcut['route']); ?>" class="group flex w-44 shrink-0 items-center justify-between gap-3 rounded-xl border px-3 py-2 shadow-sm transition <?php echo e($shortcutTone); ?>" <?php if($shortcut['active']): ?> aria-current="page" <?php endif; ?>>
                        <span class="min-w-0">
                            <span class="block truncate text-sm font-black"><?php echo e($shortcut['label']); ?></span>
                            <span class="block truncate text-xs font-semibold <?php echo e($shortcut['active'] ? 'text-white/80' : 'text-gray-500 group-hover:text-gray-700'); ?>"><?php echo e($shortcut['description']); ?></span>
                        </span>
                        <span class="rounded-full px-2 py-0.5 text-xs font-black <?php echo e($shortcut['active'] ? 'bg-white/20 text-white' : 'bg-gray-100 text-gray-800'); ?>"><?php echo e($shortcut['count']); ?></span>
                    </a>
                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
            </div>
        </nav>
        ...
        <div class="mt-3 grid gap-3 md:grid-cols-2 xl:grid-cols-4 shrink-0">
            <a href="<?php echo e($shipmentSummary['primary']['route']); ?>" class="rounded-xl border <?php echo e($shipmentSummary['primary']['tone'] === 'red' ? 'border-red-200 bg-red-50' : ($shipmentSummary['primary']['tone'] === 'emerald' ? 'border-emerald-200 bg-emerald-50' : 'border-blue-200 bg-blue-50')); ?> p-4 shadow-sm hover:shadow-md">
                <div class="flex items-start justify-between gap-3">
                    <div>
                        <p class="text-xs font-black uppercase tracking-wider <?php echo e($shipmentSummary['primary']['tone'] === 'red' ? 'text-red-700' : ($shipmentSummary['primary']['tone'] === 'emerald' ? 'text-emerald-700' : 'text-blue-700')); ?>">Accion recomendada</p>
                        <h3 class="mt-1 text-base font-black <?php echo e($shipmentSummary['primary']['tone'] === 'red' ? 'text-red-950' : ($shipmentSummary['primary']['tone'] === 'emerald' ? 'text-emerald-950' : 'text-blue-950')); ?>"><?php echo e($shipmentSummary['primary']['label']); ?></h3>
                    </div>
                    <span class="rounded-full bg-white px-2 py-0.5 text-xs font-black <?php echo e($shipmentSummary['primary']['tone'] === 'red' ? 'text-red-700' : ($shipmentSummary['primary']['tone'] === 'emerald' ? 'text-emerald-700' : 'text-blue-700')); ?>">Ver</span>
                </div>
                <p class="mt-2 text-sm font-semibold <?php echo e($shipmentSummary['primary']['tone'] === 'red' ? 'text-red-800' : ($shipmentSummary['primary']['tone'] === 'emerald' ? 'text-emerald-800' : 'text-blue-800')); ?>"><?php echo e($shipmentSummary['primary']['description']); ?></p>
            </a>

            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                <p class="text-xs font-black uppercase tracking-wider text-gray-500">Guias activas</p>
                <div class="mt-2 flex items-end justify-between gap-3">
                    <p class="text-3xl font-black text-gray-950"><?php echo e($shipmentSummary['active']); ?></p>
                    <p class="text-right text-xs font-bold text-gray-500"><?php echo e($shipmentSummary['total']); ?> en el filtro</p>
                </div>
                <div class="mt-3 h-2 rounded-full bg-gray-100">
                    <div class="h-2 rounded-full bg-blue-700" style="width: <?php echo e($shipmentSummary['total'] > 0 ? min(100, round(($shipmentSummary['active'] / $shipmentSummary['total']) * 100)) : 0); ?>%"></div>
                </div>
            </div>

            <a href="<?php echo e($shipmentSummary['attentionRoute']); ?>" class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm hover:border-red-200 hover:bg-red-50">
                <p class="text-xs font-black uppercase tracking-wider text-gray-500">Novedades</p>
                <div class="mt-2 flex items-end justify-between gap-3">
                    <p class="text-3xl font-black <?php echo e($shipmentSummary['attention'] > 0 ? 'text-red-700' : 'text-gray-950'); ?>"><?php echo e($shipmentSummary['attention']); ?></p>
                    <p class="text-right text-xs font-bold text-gray-500">Por resolver</p>
                </div>
                <p class="mt-3 text-xs font-semibold text-gray-600"><?php echo e($shipmentSummary['attention'] > 0 ? 'Prioriza llamadas o reprogramacion.' : 'Sin novedades abiertas en este filtro.'); ?></p>
            </a>

            <div class="rounded-xl border border-gray-200 bg-white p-4 shadow-sm">
                <p class="text-xs font-black uppercase tracking-wider text-gray-500">Recaudo por vigilar</p>
                <p class="mt-2 text-2xl font-black text-gray-950">$<?php echo e(number_format($shipmentSummary['collectionOpen'], 0, ',', '.')); ?></p>
                <p class="mt-3 text-xs font-semibold text-gray-600"><?php echo e($shipmentSummary['delivered']); ?> entregada(s) en este filtro.</p>
            </div>
        </div>
        ...
        <?php if($isPrintQueue && $shipments->count() > 0): ?>
                ...
                <div class="grid gap-4 xl:grid-cols-[minmax(0,1fr)_auto] xl:items-center">
                    <div class="min-w-0">
                        ...
                        <p class="mt-1 max-w-3xl text-sm font-semibold text-blue-800">
                            Hay <?php echo e($visibleCreatedCount); ?> guia(s) listas para imprimir en esta vista, con $<?php echo e(number_format($visibleCreatedValue, 0, ',', '.')); ?> en recaudo por preparar.
                        </p>
                    </div>
                    <div class="flex flex-col gap-2 sm:flex-row xl:justify-end">
                        ...
                        <a href="<?php echo e(route('shipments.index', ['status' => 'printed'])); ?>" class="inline-flex items-center justify-center rounded-lg border border-blue-200 bg-white px-4 py-2 text-sm font-black text-blue-800 shadow-sm hover:bg-blue-100">Ver impresas</a>
                    </div>
                </div>
                    ...
                    <?php $__currentLoopData = $shipments; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $shipment): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                        ...
                        <div class="sh-list-row sh-desktop-row" style="border-left:3px solid <?php echo e($barColor); ?>">
                            <div class="sh-list-cell">
                                <input type="checkbox" value="<?php echo e($shipment->id); ?>" data-shipment-status="<?php echo e($shipment->status); ?>" class="shipment-checkbox rounded border-gray-300 text-blue-700 w-4 h-4 shrink-0 aspect-square">
                            </div>
                            <div class="sh-list-cell">
                            ...
                            <div class="sh-list-cell" style="gap:3px">
                                <button type="button" data-customer-message="<?php echo e($customerMessage); ?>" onclick="copyCustomerMessage(this)" title="Copiar mensaje al cliente" class="flex items-center justify-center rounded-md border border-blue-200 bg-blue-50 w-8 h-8 text-blue-700 hover:bg-blue-100 hover:text-blue-800">
                                ...
                                <button type="button" data-tracking-url="<?php echo e($trackingUrl); ?>" onclick="copyTrackingLink(this)" title="Copiar seguimiento" class="flex items-center justify-center rounded-md border border-gray-300 bg-white w-8 h-8 text-gray-500 hover:bg-gray-50 hover:text-gray-700">
                        ...
                        <div class="sh-list-row sh-mobile-row" style="border-left:3px solid <?php echo e($barColor); ?>">
                            ...
                            <div class="sh-list-cell" style="flex-direction:column;align-items:stretch;gap:4px;flex:1">
                                ...
                                <div class="grid grid-cols-2 gap-2 mt-1 sm:grid-cols-4">
                                    <button type="button" data-customer-message="<?php echo e($customerMessage); ?>" onclick="copyCustomerMessage(this)" class="flex items-center justify-center gap-1 rounded border border-blue-200 bg-blue-50 py-2 text-xs font-semibold text-blue-700 hover:bg-blue-100">
                                    ...
                                    <button type="button" data-tracking-url="<?php echo e($trackingUrl); ?>" onclick="copyTrackingLink(this)" class="flex-1 flex items-center justify-center gap-1 rounded border border-gray-300 bg-white py-2 text-xs font-semibold text-gray-600 hover:bg-gray-50">
