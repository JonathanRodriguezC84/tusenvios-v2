...
<?php $component->withAttributes([]); ?>
     <?php $__env->slot('header', null, []); ?> 
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
            ...
            <a href="<?php echo e(route('store-settings.edit')); ?>" class="inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50">Configuracion</a>
        </div>
     <?php $__env->endSlot(); ?>
    ...
    <div class="py-6 sm:py-8">
        <div class="mx-auto max-w-6xl px-4 sm:px-6">
            <form method="POST" action="<?php echo e(route('brand-settings.update')); ?>" class="brand-settings-layout">
                <?php echo csrf_field(); ?>
                <?php echo method_field('PATCH'); ?>

                <div class="grid content-start gap-5">
                    <?php if(session('status')): ?>
                        <div class="rounded-md border border-emerald-200 bg-emerald-50 px-4 py-3 text-sm font-semibold text-emerald-800"><?php echo e(session('status')); ?></div>
                    <?php endif; ?>
                    <?php if($errors->any()): ?>
                    ...
                    <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                        ...
                        <div class="mt-4 grid gap-3 md:grid-cols-3">
                            <?php $__currentLoopData = $templates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $templateInfo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                <label class="cursor-pointer rounded-md border border-gray-200 bg-white p-4 shadow-sm transition has-[:checked]:border-blue-600 has-[:checked]:bg-blue-50">
                                    <input type="radio" name="label_template" value="<?php echo e($value); ?>" <?php if($selectedTemplate === $value): echo 'checked'; endif; ?> data-template-radio data-template-name="<?php echo e($templateInfo['name']); ?>" class="sr-only">
                                    <span class="block text-sm font-black text-gray-950"><?php echo e($templateInfo['name']); ?></span>
                                    <span class="mt-1 block text-xs leading-5 text-gray-500"><?php echo e($templateInfo['description']); ?></span>
                                </label>
                    ...
                    <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                        ...
                        <div class="mt-4">
                            <select name="default_print_format" class="block w-full rounded-lg border-gray-300 text-sm shadow-sm focus:border-blue-700 focus:ring-blue-700">
                                <?php $__currentLoopData = $printFormats; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $key => $label): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <option value="<?php echo e($key); ?>" <?php if($selectedPrintFormat === $key): echo 'selected'; endif; ?>><?php echo e($label); ?></option>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </select>
                        </div>
                    ...
                    <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                        <h3 class="mt-1 text-lg font-black text-gray-950"><?php echo e($brand['name']); ?></h3>
                        <p class="mt-2 text-sm text-gray-600">Logo, redes, color y mensaje se editan desde Configuracion.</p>
                        <a href="<?php echo e(route('store-settings.edit')); ?>" class="mt-4 inline-flex items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-2 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50">Editar datos de tienda</a>
                    </section>
                ...
                <aside class="brand-settings-preview">
                    <section class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                        <div class="flex items-start justify-between gap-3">
                            ...
                            <span id="preview-template-name" class="rounded-full bg-blue-50 px-3 py-1 text-xs font-bold text-blue-800"><?php echo e($templates[$selectedTemplate]['name'] ?? 'Clasica'); ?></span>
                        </div>

                        <div class="label-preview-scroll mt-5 rounded-lg bg-gray-100 p-3">
                            <style><?php echo file_get_contents(resource_path('views/brand-settings/partials/label-demo.css')); ?></style>
                            <div class="label-preview-fit"><div class="label-preview-fit-inner">
                                <?php $__currentLoopData = $templates; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $value => $templateInfo): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                    <div data-template-preview="<?php echo e($value); ?>" class="<?php echo e($selectedTemplate === $value ? '' : 'hidden'); ?>">
                                        <?php echo $__env->make('brand-settings.partials.label-demo', ['brand' => $brand, 'logoUrl' => $logoUrl, 'template' => $value], array_diff_key(get_defined_vars(), ['__data' => 1, '__path' => 1]))->render(); ?>
                                    </div>
                                <?php endforeach; $__env->popLoop(); $loop = $__env->getLastLoop(); ?>
                            </div></div>
                        </div>

                        <a href="<?php echo e(route('brand-settings.preview')); ?>" target="_blank" class="mt-4 inline-flex w-full items-center justify-center rounded-md border border-gray-300 bg-white px-4 py-3 text-sm font-semibold text-gray-700 shadow-sm hover:bg-gray-50">Ver etiqueta real</a>
                    </section>
                </aside>
            </form>
        </div>
...
<?php endif; ?><?php /**PATH C:\Users\Rci Shop\Herd\tusenvios_local\resources\views/brand-settings/edit.blade.php ENDPATH**/ ?>