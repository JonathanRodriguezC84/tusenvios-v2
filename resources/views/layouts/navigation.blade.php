                ...
                @foreach ($menuSections as $section)
                    ...
                    @if (! $hasChildren)
                        <a href="{{ route($section['route']) }}"
                            data-route-current="{{ $isSectionActive ? '1' : '0' }}"
                            class="flex items-center gap-3 rounded-md px-3 py-2 text-xs font-semibold transition lg:gap-2 lg:px-2 {{ $isSectionActive ? 'bg-blue-50 text-blue-800' : 'text-gray-600 hover:bg-gray-50 hover:text-gray-950' }}">
                            <svg class="h-5 w-5 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">{!! $section['icon'] !!}</svg>
                            <span>{{ $section['label'] }}</span>
                        </a>
                    @elseif ($sectionHasVisibleChildren)
                        @php
                        ...
                        <div class="pt-1">
                            <div class="flex items-center gap-3 rounded-md px-3 py-1.5 text-xs font-black uppercase tracking-wider text-gray-400 lg:gap-2 lg:px-2">
                                <svg class="h-4 w-4 shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24" aria-hidden="true">{!! $section['icon'] !!}</svg>
                                <span>{{ $section['label'] }}</span>
                            </div>
                            <div class="mt-0.5 space-y-0.5">
                                @foreach ($section['children'] as $child)
                                    ...
                                    <a href="{{ route($child['route']) }}"
                                        data-route-current="{{ $childActive ? '1' : '0' }}"
                                        class="te-menu-child-link-v01 text-xs font-semibold {{ $childActive ? 'is-active' : '' }}">
                                        <svg fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $child['icon'] !!}</svg>
        ...
        <div class="te-sidebar-user-footer border-t border-gray-200 p-4 lg:p-3">
            @if ($trialInfo)
                <div class="mb-3 rounded-lg bg-blue-50 border border-blue-200 p-2.5">
                    ...
                    <p class="text-sm font-bold text-blue-900 mt-0.5">{{ $trialInfo['remaining'] }} / {{ $trialInfo['total'] }} guias</p>
                    <div class="mt-1.5 bg-blue-200 rounded-full h-2 overflow-hidden">
                        <div class="bg-blue-600 h-full rounded-full transition-all" style="width:{{ ($trialInfo['total'] - $trialInfo['remaining']) / $trialInfo['total'] * 100 }}%"></div>
                    </div>
                    @if ($trialInfo['remaining'] <= 3)
    ...
    <nav class="mobile-bottom-nav fixed bottom-0 inset-x-0 z-40 border-t border-gray-200 bg-white lg:hidden" style="padding:6px 8px calc(6px + env(safe-area-inset-bottom));background:#ffffff;border-top:1px solid #e5e7eb;">
        <div class="mobile-bottom-grid">
            ...
            @foreach ($bottomItems as $item)
                @if (!($item['show'] ?? true)) @continue @endif
                @php $i++; $isCurrent = request()->routeIs($item['active']); @endphp
                <a href="{{ route($item['route']) }}" class="mobile-bottom-link {{ ($item['featured'] ?? false) ? 'is-featured' : '' }}" data-route-current="{{ $isCurrent ? '1' : '0' }}" data-route-create="{{ ($item['featured'] ?? false) ? '1' : '0' }}">
                    <svg class="w-[19px] h-[19px]" fill="none" stroke="currentColor" viewBox="0 0 24 24">{!! $item['icon'] !!}</svg>
