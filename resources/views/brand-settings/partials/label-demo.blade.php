...
<article class="label-sheet label-{{ $template }}">
    @if ($template === 'advance')
        <section class="label-barcode"><p class="label-guide">{{ $demo['guide'] }}</p><div class="label-code">{!! \App\Support\Code39Barcode::svg($demo['barcode'], 68) !!}</div></section>
    @endif
    ...
    <section class="label-notes"><span>OBSERVACIONES</span><strong>{{ $demo['notes'] }}</strong></section>
    ...
    <section class="label-footer"><span>TUSENVIOS.COM.CO</span><span>{{ $demo['barcode'] }}</span><span>{{ now()->format('Y-m-d H:i') }}</span></section>
</article>