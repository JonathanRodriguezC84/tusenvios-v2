<x-app-layout>
    ...
    <div class="py-8">
        <div class="mx-auto max-w-3xl px-4 sm:px-6 lg:px-8">
            <form method="POST" action="{{ route('delivery-zones.store') }}" class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm">
                @csrf
                ...
                <div class="grid gap-4 sm:grid-cols-2">
                    ...
                    <label class="grid gap-1 text-sm font-semibold text-gray-700">
                        Nombre
                        <input name="name" value="{{ old('name') }}" required placeholder="Normal" class="rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-600 focus:ring-blue-600">
                    </label>

                    <label class="grid gap-1 text-sm font-semibold text-gray-700">
                        Codigo
                        <input name="code" value="{{ old('code') }}" placeholder="NRM" class="rounded-md border-gray-300 text-sm uppercase shadow-sm focus:border-blue-600 focus:ring-blue-600">
                    </label>

                    <label class="grid gap-1 text-sm font-semibold text-gray-700">
                        Valor
                        <input name="price" type="number" min="0" step="100" value="{{ old('price', 8000) }}" required class="rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-600 focus:ring-blue-600">
                    </label>
                    ...
                    <label class="grid gap-1 text-sm font-semibold text-gray-700 sm:col-span-2">
                        ...
                        <textarea name="coverage_keywords" rows="4" placeholder="Chapinero, Teusaquillo, Barrios Unidos" class="rounded-md border-gray-300 text-sm shadow-sm focus:border-blue-600 focus:ring-blue-600">{{ old('coverage_keywords') }}</textarea>
