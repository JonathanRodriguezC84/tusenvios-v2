        ...
        <div class="mx-auto max-w-6xl px-4 sm:px-6 lg:px-8">
                ...
                <section class="mb-5 rounded-lg border border-blue-100 bg-blue-50 p-4">
                    <div class="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
                        <div>
                            ...
                            <h3 class="mt-1 text-lg font-semibold text-gray-950">Deja lista tu cuenta para imprimir</h3>
                        </div>
                        <p class="rounded-full bg-white px-4 py-2 text-sm font-semibold text-blue-800">{{ $onboarding['completed'] }}/{{ $onboarding['total'] }} completo</p>
                    </div>
                    ...
                    <div class="mt-4 grid gap-3 md:grid-cols-3">
                        @foreach ($onboarding['steps'] as $step)
                            <a href="{{ $step['route'] }}" class="rounded-md border {{ $step['done'] ? 'border-emerald-200 bg-white' : 'border-blue-100 bg-white' }} p-4">
                                <p class="text-sm font-normal text-gray-950">{{ $step['done'] ? 'Listo' : $loop->iteration.'. '.$step['label'] }}</p>
            ...
            <section class="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
                <a href="{{ route('shipments.index', ['status' => 'created']) }}" class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm hover:bg-gray-50">
                    ...
                    <p class="mt-3 text-3xl font-semibold text-gray-950">{{ $metrics['pending_print'] }}</p>
                </a>
                <a href="{{ route('shipments.index', ['status' => 'on_route']) }}" class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm hover:bg-gray-50">
                    ...
                    <p class="mt-3 text-3xl font-semibold text-gray-950">{{ $metrics['on_route'] }}</p>
                </a>
                <a href="{{ route('shipments.index', ['status' => 'delivered']) }}" class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm hover:bg-gray-50">
                    ...
                    <p class="mt-3 text-3xl font-semibold text-gray-950">{{ $metrics['delivered_today'] }}</p>
                </a>
                <a href="{{ route('shipments.index', ['status' => 'failed_delivery']) }}" class="rounded-lg border border-gray-200 bg-white p-5 shadow-sm hover:bg-gray-50">
                    ...
                    <p class="mt-3 text-3xl font-semibold text-gray-950">{{ $metrics['issues'] }}</p>
                </a>
            </section>
            ...
            <section class="mt-5 overflow-hidden rounded-lg border border-gray-200 bg-white shadow-sm">
                    ...
                    <div class="flex flex-col gap-3 border-b border-gray-200 p-5 sm:flex-row sm:items-center sm:justify-between">
                        ...
                        <a href="{{ route('shipments.index') }}" class="inline-flex items-center justify-center rounded-md border border-gray-300 px-4 py-2 text-sm font-normal text-gray-700 hover:bg-gray-50">
                            Ver todas
                        </a>
                    </div>

                    <div class="recent-shipments-head border-b border-gray-200 bg-gray-50 px-5 py-3 text-xs font-semibold uppercase text-gray-500">
                    ...
                    <div class="divide-y divide-gray-200">
                        @forelse ($shipments as $shipment)
                            ...
                            <a href="{{ route('shipments.show', $shipment) }}" class="recent-shipments-row px-5 py-4 hover:bg-gray-50">
                                <div>
                                    <p class="recent-shipments-mobile-label text-xs font-semibold uppercase text-gray-500">Fecha creacion</p>
                                    <p class="truncate text-sm font-normal text-gray-700">{{ optional($shipment->created_at)->format('d/m/Y H:i') }}</p>
                                </div>
                                <div>
                                    <p class="recent-shipments-mobile-label text-xs font-semibold uppercase text-gray-500">Nombre</p>
                                    <p class="truncate text-sm font-normal uppercase text-gray-950">{{ $shipment->recipient_name }}</p>
                                </div>
                                <div>
                                    <p class="recent-shipments-mobile-label text-xs font-semibold uppercase text-gray-500">Estado</p>
                                ...
                                <div>
                                    <p class="recent-shipments-mobile-label text-xs font-semibold uppercase text-gray-500">Precio</p>
                                    <p class="truncate text-sm font-normal text-gray-950">${{ number_format((float) $priceValue, 0, ',', '.') }}</p>
                                </div>
                                <div>
                                    <p class="recent-shipments-mobile-label text-xs font-semibold uppercase text-gray-500">Guia</p>
