@php
    ...
    $selectedPrintFormat = $selectedPrintFormat ?? '100x150';
    $printFormats = $printFormats ?? [$selectedPrintFormat => $printFormat];
...
<html lang="es">
    <head>
        ...
        <style>
            ...
            * { box-sizing: border-box; }
            ...
            .actions { margin: 14px auto; max-width: 760px; padding: 0 14px; text-align: center; }
            button { background: {{ $brand['color'] ?? '#022a8c' }}; border: 0; border-radius: 7px; color: #fff; cursor: pointer; font-size: 14px; font-weight: 800; padding: 10px 14px; }
            .format-links { display: flex; flex-wrap: wrap; gap: 8px; justify-content: center; margin-top: 10px; }
            .format-links a { border: 1px solid #cbd5e1; border-radius: 7px; color: #111827; font-size: 12px; font-weight: 800; padding: 7px 10px; text-decoration: none; }
            .format-links a.active { background: #111827; border-color: #111827; color: #fff; }
            .print-page {
                align-items: center;
                background: #ffffff;
                display: flex;
            ...
            .print-label {
                ...
                max-width: calc({{ $printFormat['width'] }} - {{ $printFormat['padding'] }} * 2);
                max-height: calc({{ $printFormat['height'] }} - {{ $printFormat['padding'] }} * 2);
            }
            @media print {
                * {
 ...
 body { background: #fff; } .actions { display: none; } .print-page { margin: 0; } }
    ...
    <body>
        <div class="actions">
            <button onclick="window.print()">Imprimir {{ $printFormat['short_label'] }}</button>
            <div class="format-links" aria-label="Formatos de impresion">
                @foreach ($printFormats as $formatKey => $format)
                    <a href="{{ route('shipments.print', ['shipment' => $shipment, 'format' => $formatKey]) }}" onclick="event.preventDefault(); window.open(this.href, 'print{{ $shipment->id }}{{ $formatKey }}', 'width=800,height=600,scrollbars=yes,resizable=yes')" class="{{ $selectedPrintFormat === $formatKey ? 'active' : '' }}">
                        {{ $format['short_label'] }}
                    </a>
                @endforeach
            </div>
